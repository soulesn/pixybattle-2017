...
while True:
    try:
        ...
        print
        "Could not open serial device %s" % serialDevice
        time.sleep(10)
...
refDist = 1000
# list of behaviors to execute in sequence
behaviors = list()
...
explore_behavior_1 = {'timeToExecute': 1.5, 'bias': 1, 'diffDrive': 0.5, 'throttle': 0.4, 'advance': 1}
explore_behavior_2 = {'timeToExecute': 1.5, 'bias': 0, 'diffDrive': 0, 'throttle': 0.6, 'advance': 1}
...
def setup():
    ...
    if pixy_init_status != 0:
        print
        'Error: pixy_init() [%d] ' % pixy_init_status
...
def loadBehavior(behaviorList):
    global previousBenchTime, behaviors, currentBehaviorIndex
...
def executeBehavior():
    ...
    if (previousBenchTime != None):
        ...
        behaviorTime = behaviors[currentBehaviorIndex]['timeToExecute']
...
def loop():
    ...
    if ((
        ...
        panError = PIXY_X_CENTER - (blocks[
                                        targetBlockIndex].x)  # +int(35*math.sin(5*targetTimeDifference))) #100 - blocks[biggestGreenBlockIndex].x
    ...
    if panLoop.m_pos != PIXY_RCS_CENTER_POS:
        ...
        bias = float(turnError) / float(PIXY_RCS_CENTER_POS) * h_pgain + i_control * float(
            turnErrorAccumulator) / float(PIXY_RCS_CENTER_POS) + d_control * derivative
